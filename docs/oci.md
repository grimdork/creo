...
creo uses the platform user config directory for caches and
persistent data:

| Platform | Directory |
|---|---|
| Linux | `~/.config/creo/` |
| macOS | `~/Library/Application Support/creo/` |

This document refers to this directory as `$CREO_DIR`.
...
| `from=` | Base image to layer the binary on; pulls and caches to `$CREO_DIR/oci/` |
...
`$CREO_DIR/oci/`.  The cache is valid for 24 hours; stale entries
